...
router.get("/:email", async (req, res) => {
  ...
  try {
    console.log(`Fetching profile for email: ${email}`); // Debug log corrected

    // Corrected the regex usage for email matching
    const coordinator = await Coordinator.findOne({
      email: new RegExp(`^${escapedEmail}$`, "i"),
    });

    if (!coordinator) {
      console.log("Coordinator not found in database"); // Debug log
      return res.status(404).json({ message: "Coordinator not found" });
    }

    res.json(coordinator); // Send coordinator details as response
  } catch (err) {
...
router.put("/:email/update-image", async (req, res) => {
  const { email } = req.params;
  const { image } = req.body; // New image for the coordinator
